...
def plot_reference_dark(path, dark_filename, reference_filename):
    ...
    plt.plot(dark['Wavelength'],
             (reference['Sp mean'] + reference['Sn mean']) / 2 - (dark['Sp mean'] + dark['Sn mean']) / 2,
             label='ref - dark')
    plt.xlabel(r'$\lambda$')
    plt.ylabel('Count')
    # plt.title('(Sp + Sn)/2')
...
def compute_absorbance(path, dark, reference, signal_filename):
    ...
    A = -0.5 * (np.log10(signal['Sp mean'] - dark['Sp mean']) + np.log10(
        signal['Sn mean'] - dark['Sp mean']) - 2 * np.log10(reference['Sp mean'] - dark['Sp mean']))
