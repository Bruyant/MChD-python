...
if winspecTypeLib is None:
    raise ImportError(
        "The Winspec type library was not found. If using windows 7 then try running winspec once as an administrator")
...
class Winspec(object):
   ...
   This software was last tested with Winspec v2.6.22.0. Older versions of Winpec may have some bugs. """
    ...
    def __init__(self):
        ...
        self.setExpSetupProfile("spectrometer_config/automatic.EXS")
        # Read the detector calibration data necessary to do a step and glue
        # try:
        #     self.detector = readDetectorDefinition()
        # except IOError:
        #     self.detector = {"calibrationRange": DEFAULT_CAL_RANGE, "width": self.getNumberOfPixels(),
        #                      "height": self.getDetectorHeight()}


        # print(self.setSpectroscopyMode(mode=False))
    ...
    def _getWinspecSpectrum(self, numFrames=1):
        ...
        if not READ_FROM_FILE:
            ...
            if (fname.lower()[-4:]) != ".spe": fname = fname + ".spe"
        ...
        avgCounts[avgCounts < 0] = 0  # make sure no entries are below zero in final data # TODO: check if ok ?
    ...
    def getDispersionCalibration(self):
        ...
        return (
        cal["centerFromLeft"][g], cal["centerFromRight"][g], cal["leftFromCenter"][g], cal["rightFromCenter"][g])
    ...
    def measureCalibration(self):
       ...
       Works by scanning the spectrometer through some wavelength range, and extracting min and max wavelengths from dummy data """
        ...
        for grating in [0, 1, 2]:
            ...
            self.detector["resolution"][grating] = (polyval(rfc, mean(calRange)) - polyval(lfc, mean(calRange))) / \
                                                   self.detector["width"]
